...
# -------------------------
# Definizione del Modello Kanet5 (LeNet5_KAN)
# -------------------------
class LeNet5_KAN(nn.Module):
    def __init__(self, num_classes=62):  # EMNIST ha 62 classi
        super(LeNet5_KAN, self).__init__()
        # Primo strato conv KAN: in_channels=1, out_channels=6, kernel=5x5
        ...
        # Secondo strato conv KAN: in_channels=6, out_channels=16, kernel=5x5
        ...
        # Nota: dopo le convoluzioni e pooling, la dimensione spaziale è 4x4 (non 5x5 come in LeNet5 standard)
        self.fc1 = nn.Linear(16 * 4 * 4, 120)
        self.fc2 = nn.Linear(120, 84)
        self.fc3 = nn.Linear(84, num_classes)

        # Variabile per salvare le feature maps dopo conv2
    ...
    def forward(self, x):
        ...
        x = self.pool1(x)
        x = self.conv2(x)
        self.last_conv_feat = x  # Dimensione attesa: [B, 16, 8, 8]
        x = self.pool2(x)
...
def main():
    ...
    model_variants = [
        {"norm_type": "L2", "grid_size": 5, "spline_order": 3},
        {"norm_type": "None", "grid_size": 5, "spline_order": 3}
    ]
    ...
    for variant in model_variants:
        ...
        # Modifica del percorso: utilizziamo 'KaNet5' invece di 'Standard_LeNet5'
        ...
        for ckpt_file in checkpoint_files:
            ...
            # ============================
            # Per FeatureMap – dimensione originale (8x8 per Kanet5)
            heatmap_orig_accum_by_class_train = {cls: torch.zeros((8, 8), device=device) for cls in range(num_of_classes)}
            heatmap_orig_squared_accum_by_class_train = {cls: torch.zeros((8, 8), device=device) for cls in range(num_of_classes)}
            heatmap_orig_count_by_class_train = {cls: 0 for cls in range(num_of_classes)}
            heatmap_orig_accum_by_class_test = {cls: torch.zeros((8, 8), device=device) for cls in range(num_of_classes)}
            heatmap_orig_squared_accum_by_class_test = {cls: torch.zeros((8, 8), device=device) for cls in range(num_of_classes)}
            ...
            # Per GradCAM – originale (8x8, ottenute downscalando il risultato upsampled)
            gradcam_orig_accum_by_class_train = {cls: torch.zeros((8, 8), device=device) for cls in range(num_of_classes)}
            gradcam_orig_squared_accum_by_class_train = {cls: torch.zeros((8, 8), device=device) for cls in range(num_of_classes)}
            gradcam_orig_count_by_class_train = {cls: 0 for cls in range(num_of_classes)}
            gradcam_orig_accum_by_class_test = {cls: torch.zeros((8, 8), device=device) for cls in range(num_of_classes)}
            gradcam_orig_squared_accum_by_class_test = {cls: torch.zeros((8, 8), device=device) for cls in range(num_of_classes)}
            ...
            def process_loader(loader, is_test=False):
                for data, target in loader:
                    ...
                    with torch.no_grad():
                        ...
                        feats = model.last_conv_feat  # [B, 16, 8, 8]
                        B = feats.size(0)
                        # Calcola la heatmap: media sui canali
                        heatmaps_orig = torch.mean(feats, dim=1)  # [B, 8, 8]
                    ...
                    # Ottieni la versione originale (8x8) downscalando (adaptive_avg_pool2d)
                    gradcam_maps_orig = F.adaptive_avg_pool2d(gradcam_maps_up.unsqueeze(1), output_size=(8, 8)).squeeze(1)
            ...
            for cls in range(num_of_classes):
                # --- FeatureMap: Versione ORIGINALE (8x8) ---
                ...
                if heatmap_up_count_by_class_test[cls] > 0:
                    ...
                    print(f"Salvata variance featuremap (Upsampled, test) per classe {cls}: {img_filename}")
            ...
            for cls in range(num_of_classes):
                ...
                if gradcam_up_count_by_class_test[cls] > 0:
                    ...
                    print(f"Salvata variance GradCAM (Upsampled, test) per classe {cls}: {img_filename}")

                # --- GradCAM: Versione ORIGINALE (8x8) ---
                ...
                if gradcam_orig_count_by_class_test[cls] > 0:
                    ...
                    print(f"Salvata variance GradCAM (Original, test) per classe {cls}: {img_filename}")
