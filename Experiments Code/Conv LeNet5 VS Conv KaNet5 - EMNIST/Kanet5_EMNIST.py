...
from kan_convolutional.kanet5_EMNIST import LeNet5_KAN
...
#model = LeNet5_KAN(num_classes=62).to(device)  # E-MNIST ha 62 classi
model = LeNet5_KAN(num_classes=62)
model = torch.jit.script(model).to(device)

train_and_evaluate(model, 'KaNet5', [0, 0], "KaNet5", learning_rate, epochs, norm_type,  grid_size, spline_order, len(set(labels_train)))
