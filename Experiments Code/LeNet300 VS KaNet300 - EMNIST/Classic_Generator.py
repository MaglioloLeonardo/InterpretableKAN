...
train_and_evaluate(model_output_only, 'LeNet300_OutputOnly', [784, 10], "This model uses only the output layer.", learning_rate, epochs, norm_type,  grid_size, spline_order, len(set(labels_train)))

model_hidden_only = LeNet300_HiddenLayerOnly().to(device)
train_and_evaluate(model_hidden_only, 'LeNet300_HiddenLayerOnly', [784, 300, 10], "This model uses a single hidden layer with 300 neurons.", learning_rate, epochs, norm_type,  grid_size, spline_order, len(set(labels_train)))

#model_full = LeNet300_Full().to(device)
#train_and_evaluate(model_full, 'LeNet300_Full', [784, 300, 100, 10], "This is the full LeNet-300-100 architecture.", learning_rate, epochs, norm_type,  grid_size, spline_order, len(set(labels_train)))
