...
class LeNet300_OutputOnly(nn.Module):
    ...
    def __init__(self):
        ...
        self.fc = nn.Linear(784, 62)  # Only input layer and output layer
    ...
    def forward(self, x):
        ...
        return F.log_softmax(x, dim=1)  # Apply Log-Softmax on tensor output
...
class LeNet300_HiddenLayerOnly(nn.Module):
    ...
    def __init__(self):
        ...
        self.fc2 = nn.Linear(300, 62)   # 300 hidden layer -> Output layer
        self.relu = nn.ReLU()           # Define ReLU explicitly
...
class LeNet300_Full(nn.Module):
    ...
    def __init__(self):
        ...
        self.fc3 = nn.Linear(100, 62)   # 100 hidden layer -> Output layer
        self.relu = nn.ReLU()           # Define ReLU explicitly
...
def generate_basedir_name(optimizer, grid_size, spline_order, lr, norm_type):
    ...
    base_dir = f"results_{norm_suffix}_{optimizer_name}_lr{lr}_0_0"
    return base_dir
...
model_output_only = KANLinearFullyConnected([784, 62], grid_size = grid_size, spline_order = spline_order).to(device)
train_and_evaluate(model_output_only, 'LeNet300_OutputOnly', [784, 62], "This model uses only the output layer.", learning_rate, epochs, norm_type,  grid_size, spline_order)

model_hidden_only = KANLinearFullyConnected([784, 300, 62], grid_size = grid_size, spline_order = spline_order).to(device)
train_and_evaluate(model_hidden_only, 'LeNet300_HiddenLayerOnly', [784, 300, 62], "This model uses a single hidden layer with 300 neurons.", learning_rate, epochs, norm_type,  grid_size, spline_order)

model_full = KANLinearFullyConnected([784, 300, 100, 62], grid_size = grid_size, spline_order = spline_order).to(device)
train_and_evaluate(model_full, 'LeNet300_Full', [784, 300, 100, 62], "This is the full LeNet-300-100 architecture.", learning_rate, epochs, norm_type,  grid_size, spline_order)
