...
class LeNet300_OutputOnly(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.relu(self.fc(x))    # Apply ReLU after fully connected layer
        return F.log_softmax(x, dim=1)  # Apply Log-Softmax on ReLU output
...
class LeNet300_HiddenLayerOnly(nn.Module):
    ...
    def forward(self, x):
        x = x.view(x.size(0), -1)     # Flatten the MNIST image (28x28 -> 784)
        x = self.relu(self.fc1(x))    # ReLU activation for hidden layer
        x = self.relu(self.fc2(x))    # ReLU activation for output layer
        return F.log_softmax(x, dim=1)  # Apply Log-Softmax on ReLU output
...
class LeNet300_Full(nn.Module):
    ...
    def forward(self, x):
        x = x.view(x.size(0), -1)     # Flatten the MNIST image (28x28 -> 784)
        x = self.relu(self.fc1(x))    # ReLU activation for first hidden layer
        x = self.relu(self.fc2(x))    # ReLU activation for second hidden layer
        x = self.relu(self.fc3(x))    # ReLU activation for output layer
        return F.log_softmax(x, dim=1)  # Apply Log-Softmax on ReLU output
'''
...
#model_output_only = KANLinearFullyConnected([784, 10], grid_size = grid_size, spline_order = spline_order).to(device)
...
model_hidden_only = KANLinearFullyConnected([784, 300, 10], grid_size = grid_size, spline_order = spline_order).to(device)
...
model_full = KANLinearFullyConnected([784, 300, 100, 10], grid_size = grid_size, spline_order = spline_order).to(device)
